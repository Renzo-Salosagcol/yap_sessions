...
export function ChatPage({ activeChat }: { activeChat: number | null }) {
  ...
  const addMessage = () => {
    const setActiveInput = document.getElementById("message-input").value
    const date = new Date()
    testMessages.push({
      user: "user 1",
      message: "dick",
    ...
    <ScrollArea className="min-h-screen w-full flex flex-col items-center justify-between gap-4 p-2 py-0 pb-5 glassmorphism">
      <div className="w-full minh-screen h-full p-4 mr-4 bg-background rounded-lg shadow-lg overflow-y-auto">
        ...
        <div className="bottom-0 left-0 w-full p-2 fixed flex flex-row items-center justify-center gap-4 glassmorphism">
          ...
          <Button onClick={addMessage} className="gradient p-5 flex flex-row items-center gap-2" size="sm">Send <Send /></Button>
        </div>
      </div>
